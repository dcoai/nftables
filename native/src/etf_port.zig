...
/// Read a packet-length-prefixed ETF message from stdin
/// Returns allocated buffer containing the ETF message
...
/// Write a packet-length-prefixed ETF message to stdout
...
pub fn main() !void {
    ...
    // Main loop: Read ETF commands, convert to JSON, execute, convert response to ETF
    while (true) {
        // Read packet-length-prefixed ETF command
        const etf_cmd = readPacket(allocator, stdin_file) catch |err| {
        ...
        defer allocator.free(etf_cmd);

        // Convert ETF to JSON
        const json_cmd = etfToJson(allocator, etf_cmd) catch |err| {
            std.debug.print("ERROR: Failed to convert ETF to JSON: {}\n", .{err});

            // Send error response as ETF
            const error_json = "{\"error\": \"ETF decode failed\"}";
            const error_etf = try jsonToEtf(allocator, error_json);
            defer allocator.free(error_etf);
            try writePacket(stdout_file, error_etf);
            continue;
        };
        ...
        defer allocator.free(response_etf);

        // Send ETF response back to Elixir
        try writePacket(stdout_file, response_etf);
